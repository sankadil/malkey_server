...
<%@page import="com.dspl.malkey.service.FmaintenanceSRV"%>
...
<%
	...
	Object reportSource =null; //session.getAttribute("Report7");
	    ...
	    FmaintenanceSRV srv = (FmaintenanceSRV) wac.getBean("fmaintenanceSRV");
		...
		dateFrom=request.getParameter("df").toString();
		dateTo=request.getParameter("dt").toString();
		maintFrequency=request.getParameter("mf").toString();
		if(maintFrequency!=null && maintFrequency.trim().length()>0 && maintFrequency.toLowerCase().equals("all")==false){
			...
			paraList.add(aso);
			if(maintFrequency.toLowerCase().equals("recurring")==true)
				maintType=request.getParameter("mt").toString();
		}
		...
		maintStatus=request.getParameter("ms").toString();
		...
		List<Fmaintenance> list= srv.getMaintenanceReminder(dateFrom,dateTo,paraList);
...
<%@page import="com.dspl.malkey.domain.Fmaintenance"%>
